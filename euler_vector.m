classdef euler_vector
    % euler_vector: class to calculate and plot an Euler vector of rotation 
    ...
    % determine the Euler vector. Finally, a name for the vector 
    % (also optional)
    % 
    ...
    %
    % The vector will be calculated using all stations or using a subset
    % determined by the either a cell array with the stations names
    % or another text file with a list of station names.
    % The Euler vector is determined using the technique explained in 
    % Smalley et al. (xxxx) and a robost least squares algorithm 
    ...
    properties
        ...
        wrms_n
        wrms_e
        vector_name % better to reference the vector itself, not the figure
    end
    
    methods
        function self = euler_vector(varargin)
            ...
            if nargin > 1
                ...
                if ~isempty(varargin{2})
                    ...
                    vector_stations = varargin{2};
                    
                    if ~iscell(vector_stations)
                        if isfile(vector_stations)
                            % file with station list, load it
                            vector_stations = table2cell(readtable(vector_stations));
                        elseif ischar(vector_stations)
                            vector_stations = cellstr(vector_stations);
                        end
                    end
                end
            end
            ...
            if isfolder(stations_input)
                ...
                for i = 1:length(self.json_files)
                    % if stations exists in vector_stations or vector_stations
                    % is empty, read the json file
                    if or(sum(~cellfun(@isempty, regexp(lower(self.json_files(i).name), lower(vector_stations)))), isempty(vector_stations))
            ...
            elseif isfile(stations_input)
                ...
                j = 1;
                for i = 1:height(data)
                    if or(sum(~cellfun(@isempty, regexp(lower(table2cell(data(i, 1))), lower(vector_stations)))), isempty(vector_stations))
            ...
            % now calculate the Euler vector
            self = self.calculate_vector();
        end
        
        function self = calculate_vector(self)
            % Weigther Euler vector calculation
            ...
            if ~pass
                warning(['Goodness of fit test for Euler vector ' self.vector_name ' did not converge!'])
            end
            ...
            % save the residuals of the vector calculation
            ...
            % save the vector as is
            self.vector_X = X;
            
            % convert euler vector to lat lon position
            ...
            % Propagate the Euler vector covariance 
        ...
        function plot_ellipse(self, cov, lla, vn, ve, scale, color)
            ...
            plot(r_ellipse(:,1) + x + mVe * scale, ...
                 r_ellipse(:,2) + y + mVn * scale, 'color',  color)
        end
    end
    ...
    methods(Static)
        ...
        function [x, lla] = xyz_lla(lla_or_x)
            ...
            if n(1) > 6000e3
                ...
                x = euler_vector.g2x([lla(:, 1:2)'; lla_or_x(:, 3)' + 6370e3]);
                x = x';
            end    
        end
        ...
        function x = determine_input(x)
            ...
            if iscell(x)
                ...
                for i = 1:length(x)
                    stn(i) = euler_vector.load_json(x{i});
                end
                x = [stn(:).lat; 
                     stn(:).lon; 
                     zeros(1, i); 
